...
- **Carrier localization.** A hit is reported at the carrier, not at the
  ...
  offset within a probe, removing the ±2.5 MHz grid quantization that
  cross-sweep integration cannot average away. The vertical-sync confirm
  stage re-runs on that same whole-swing cut, so reaching the confirmed
  tier does not depend on where the probe grid falls relative to the
  carrier (§3.1).
- **Clustering and scoring.** Detections are anchored on the strongest
  hit and merged when within ±12.5 MHz of it, then scored from 0.0 to
  1.0.
...
- **Burst-gated subcarrier notch.** The dot-crawl notch engages only when
  ...
  effectively monochrome, and on those an ungated notch removes real luma
  detail.
