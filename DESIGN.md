...
2. **DDC + Decimation**: At each probe position, a Digital Down-Converter (NCO mixer) shifts the probe frequency to DC, then a 63-tap Blackman-windowed-sinc FIR (> 50 dB stopband, cutoff `target_rate / 3`; see §6 item 1 for why a boxcar will not do) band-limits before integer-stride decimation to 10 MSPS. This isolates a slice around each probe center.
...
6b. **Tip timing**: both classifiers place a tip at the **midpoint of its sub-threshold run** (item 5 explains why the run's lowest sample scatters the intervals), skip a run cut off by either end of the scan window, and never step back into the run just measured — a broad pulse stretched wider than the 30 µs skip would otherwise re-enter as a phantom tip. That last rule also keeps the scan terminating at any sample rate, where the skip itself truncates to zero. The failure this guards against was platform-dependent: on a clean synthetic NTSC record the argmin's scatter left 16 of 78 intervals inside ±1 % of the median on one machine's FIR rounding and passed on another's, which is how `tests/gpu_equivalence.rs` was red on CI's x86 runners and green on Apple Silicon. `ANALOG_PROBE=1` prints `TD:` trace lines from `classify_pal_ntsc_time_domain` saying which gate a record fails — threshold, run shape, interval count, consensus, line-rate range, the ±30 Hz dead-band — and the numbers behind it; the flag is read once per process.
...
The sliding sweep classifies probes on a 5 MHz grid. Reporting a hit
*at the probe centre* carries a uniform ±2.5 MHz error, and because it
depends only on where the grid falls relative to the signal, it is the
same error on every sweep — `SpectralIntegrator` cannot average it
away. A synthetic carrier measured a constant −0.7 MHz across every
sweep.
...
Clustering follows from it. Sweep hits are grouped within 25 MHz.
Anchoring the group on the lowest-frequency hit does not work: in
integrated mode (every probe classified, §11) that can be a noise-floor
probe at the band edge, giving a window that holds a signal's own probes
while excluding the weaker sibling that classified it, so the two halves
fail separately. Clusters anchor on the strongest hit and admit members
within ±12.5 MHz of it — half the 25 MHz window, so a
...
## 6. Sweep Design Notes

No open follow-ups. Two constraints on the sweep are recorded here
because both are easy to reintroduce.

1. **`ddc_and_decimate` needs a real FIR, not a boxcar.** A length-N
   boxcar (`sum/N`) low-pass has a sinc magnitude response and poor
   stopband attenuation, so under the FM threshold effect adjacent-band
   energy leaks through and synthesises spurious harmonic content in the
   discriminator output. The 63-tap Blackman-windowed-sinc FIR
   (> 50 dB stopband) closes that at the cost of one extra allocation
   per probe. `process_into_decimated` runs the convolution only on
   decimation-aligned samples; the mixer and delay-line writes that
   remain are per-sample and irreducible in this structure.
2. **Sweep decimation must track the actual rate, not the nominal
   one.** Integer division of `sample_rate / target_rate` yields an
   exact rate only when `sample_rate` is a clean multiple of
   `target_rate` (50 or 100 MSPS). At 25 MSPS the factor truncates from
   2.5 to 2, giving 12.5 MHz of actual output; passing the nominal
   `WIDEBAND_TARGET_RATE_HZ` into `detect_sync_pulses` instead silently
   corrupts every frequency-derived computation there. The
   `decimated_rate` / `decimation_factor` helpers compute it the same
   way the DDC does, so the two cannot drift apart.
...
- **Smoothed AGC**: the per-line sync-tip/porch measurements drive an
  ...
  clamped to 0.25–4×) rather than being applied instantaneously:
  applied instantaneously, one click in a 25-sample porch window swings
  an entire line's brightness. Lines with no valid swing reuse the last good state;
...
Two independent parts of sync acquisition must not assume the demod
output arrives at exactly its nominal scale and phase. That assumption
fails by losing *every* row of *every* field at once, which is far
harder to diagnose than a gradual degradation.
