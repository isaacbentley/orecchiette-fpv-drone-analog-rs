...
//! - discriminator vs PLL vs UKF demod fidelity
...
use orecchiette_fpv_drone_analog_rs::demod::{fm_demod, PllFmDemod};
...
use orecchiette_fpv_drone_analog_rs::synthetic::{
    generate_fields, generate_iq, SyntheticVideoConfig, TestPattern,
};
...
fn main() {
    ...
    let mut offset = 1;
    for _ in 0..2 {
        if let Some(consumed) = truth_recon.reconstruct_frame_into(&truth[offset..], &mut truth_frame) {
            offset += consumed;
        } else {
            break;
        }
    ...
    for &impairment in &[Impairment::Awgn, Impairment::Multipath, Impairment::BurstDropout, Impairment::SlowFade, Impairment::ImpulsiveNoise] {
        println!("\n=== Profile: {:?} ===", impairment);
        println!(
            "{:>5} | {:>8} | {:>12} | {:>16} | {:>10} {:>10} | {:>5} {:>5} | {:>5} {:>5}",
            "sigma", "CNRest", "single(4x)", "integrated(4)",
            "disc MSE", "pll MSE",
            "syncD", "syncP",
            "gcorD", "gcorP"
        );
        ...
        for &sigma in &[0.0f32, 0.3, 0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 1.8, 2.2] {
            ...
            match impairment {
                ...
                Impairment::Multipath => {
                    orecchiette_fpv_drone_analog_rs::impairments::add_multipath(&mut iq, &[(15, 0.5, 1.0, 50.0), (40, 0.2, 0.5, 200.0)], sample_rate as f32);
                }
                Impairment::BurstDropout => {}
                Impairment::SlowFade => {
                    orecchiette_fpv_drone_analog_rs::impairments::add_slow_fade(&mut iq, sample_rate as f32, 60.0, 0.8);
                }
                Impairment::ImpulsiveNoise => {
                    orecchiette_fpv_drone_analog_rs::impairments::add_impulsive_noise(&mut iq, 0.05, 5.0, &mut seed);
                }
            }
            add_awgn(&mut iq, sigma, &mut seed);
            if matches!(impairment, Impairment::BurstDropout) {
                orecchiette_fpv_drone_analog_rs::impairments::add_burst_dropouts(&mut iq, &[(10000, 2000), (50000, 1000)], sigma, &mut seed);
            }
            ...
            for b in 0..4u64 {
                ...
                match impairment {
                    Impairment::Awgn => {}
                    Impairment::Multipath => orecchiette_fpv_drone_analog_rs::impairments::add_multipath(&mut iq_b, &[(15, 0.5, 1.0, 50.0), (40, 0.2, 0.5, 200.0)], sample_rate as f32),
                    Impairment::BurstDropout => {}
                    Impairment::SlowFade => orecchiette_fpv_drone_analog_rs::impairments::add_slow_fade(&mut iq_b, sample_rate as f32, 60.0, 0.8),
                    Impairment::ImpulsiveNoise => orecchiette_fpv_drone_analog_rs::impairments::add_impulsive_noise(&mut iq_b, 0.05, 5.0, &mut s),
                }
                add_awgn(&mut iq_b, sigma, &mut s);
                if matches!(impairment, Impairment::BurstDropout) {
                    orecchiette_fpv_drone_analog_rs::impairments::add_burst_dropouts(&mut iq_b, &[(10000, 2000), (50000, 1000)], sigma, &mut s);
                }

                let (st, conf) = det.detect_sync_pulses(&iq_b, sample_rate);
                if st.is_analog_video() {
                    hits += 1;
                    if conf > best.1 { best = (st, conf); }
                }
            }
            ...
            let eval_demod = |demod: &[f32], phase1: bool, name: &str| -> (String, f64) {
                ...
                for _ in 0..2 {
                    if let Some(consumed) = recon.reconstruct_frame_into(&demod[offset..], &mut frame) {
                        offset += consumed;
                        processed_any = true;
                    } else {
                        break;
                ...
                if processed_any {
                    ...
                    if (sigma - 0.5).abs() < 1e-3 {
                        ...
                        use std::io::Write;
                        if let Ok(mut f) = std::fs::File::create(format!("target/frames/{:?}_{}_{}.ppm", impairment, sigma, name)) {
                            writeln!(f, "P3\n{} {}\n255", recon.width, recon.height).unwrap();
                            for &px in &frame {
                                writeln!(f, "{} {} {}", (px >> 16) & 0xFF, (px >> 8) & 0xFF, px & 0xFF).unwrap();
                            }
                        }
                    }
                    
                    let grad_corr = orecchiette_fpv_drone_analog_rs::metrics::compute_gradient_correlation(&frame, &truth_frame, recon.width, recon.height);
            ...
            if gcor_disc < 0.5 && cliff_disc.is_none() { cliff_disc = Some(sigma); }
            if gcor_pll < 0.5 && cliff_pll.is_none() { cliff_pll = Some(sigma); }
        ...
        println!("---");
        println!("Disc σ-cliff: {}", cliff_disc.map(|v| format!("{v:.2}")).unwrap_or_else(|| ">2.2".into()));
        println!("PLL  σ-cliff: {}", cliff_pll.map(|v| format!("{v:.2}")).unwrap_or_else(|| ">2.2".into()));
    }
}
