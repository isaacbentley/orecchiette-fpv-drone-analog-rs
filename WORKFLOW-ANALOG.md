...
1.  **Multi-Band Orchestration & Auto-Scanning**:
    - **Frequency Pool**: The system can scan a consolidated list of channels, including Band A, B, E (Boscam), F (FatShark), R (RaceBand), L (LowBand), and D (Boscam D / "5.3G") at 5.8 GHz, plus the 1.2 GHz tables (the amateur / SM1370R 1240–1300 MHz set and the 1080–1360 MHz long-range VTX grid) and the 64-channel 3.3 GHz grid.
    ...
    - **Dwell**: the sweep dwells **20 ms per hop** (USRP; 15 ms while fine-tuning, 500 ms on the Aaronia HTTP backend, where a retune is a config PUT plus a stream settle rather than a synthesiser hop). The detector needs **two** chunks per hop, not one: `SpectralIntegrator` accumulates magnitude spectra per frequency, and a single 65536-sample chunk is below the length at which the sweep detects anything, so one chunk per hop pushes first detection to the second visit. Two chunks is ~5.2 ms of signal at 25 MSPS, so 20 ms leaves margin at every rate the scan runs. Remaining duplicate-frequency packets are drained to prevent queue backlog.
...
2.  **Zero-Allocation Pipeline**:
    ...
    - **Scan-loop backpressure**: the fpv-viewer scan loop processes `DETECT_PACKETS_PER_HOP` (2) packets per frequency hop and drains the rest at the same centre frequency. This prevents queue buildup when the detector's DDC sweep cannot keep up with the packet rate; a hop is bound by detection CPU (~18 ms for two passes) rather than by the dwell.
...
7.  **H-Sync Rate Detection (FFT)**:
    ...
    - At coarse resolution (e.g., 2.6 ms at 100 MSPS, or a 65 k chunk at 25 MSPS ≈ 381 Hz/bin), PAL and NTSC bins collide. The detector then falls back to a **time-domain median sync-tip interval** (`classify_pal_ntsc_time_domain`): it counts sync tips on the (smoothed) demodulated record against a robust percentile threshold (`p2 + 0.25·(p50−p2)` — DC-invariant and immune to single FM-click outliers, where a global-minimum threshold is neither), takes the median line period, and maps it to a line rate compared against 15625/15734 Hz with a ±30 Hz midpoint dead-band — classifying PAL/NTSC at confidence 0.8. Only if that's inconclusive (too few tips, or median in the dead-band) does it tag `AnalogVideoUnknown` at 0.6 (rather than silently picking one standard). `SignalType::is_analog_video()` returns `true` in all three cases, so downstream consumers gating on "analog FPV present" still see the hit.
...
10. **Signal Clustering**:
    - All positive detections from the probe sweep are sorted by **energy, strongest first**, so a cluster anchors on its strongest hit rather than on whichever is lowest in frequency. Anchoring on the lowest lets a noise-floor probe at the band edge decide where the window falls.
    - A hit joins a cluster when it lies within **±12.5 MHz** of that cluster's anchor (`CLUSTER_BW_HZ / 2`, a 25 MHz total window). Using the full 25 MHz as the radius would span 50 MHz and collapse three transmitters 24 MHz apart into one report.
    - The anchor — the strongest member — is the representative detection.
...
12. **Quadrature Demodulation** (or PLL — `demod::PllFmDemod` offers
    ...
    - **Implementation**: exact scalar `f32::atan2` (via `Complex::arg`) per sample. The complex multiply + `conj` ahead of the atan2 auto-vectorises cleanly under `-O3` (it's the bulk of the work). The atan2 is deliberately not a polynomial approximation: approximate kernels (e.g. `fast_math::atan2`) lose precision near ±π, exactly where high-deviation FM operates, and the resulting quadrant errors show up as click-noise sparkles in the reconstructed picture. The throughput gain does not pay for that.
