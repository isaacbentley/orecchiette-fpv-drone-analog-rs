...
pub struct FrameReconstructor {
    ...
    ///
    /// The alternative — `line_doubling`, copying each captured row
    /// into both even and odd output positions — throws away half of
    /// the vertical detail. The terminal output's
    ...
    /// measurements come from two ~25-sample window means. Without the
    /// EMA, at low CNR a single click in the porch window swings an
    /// entire line's brightness, visible as line-to-line flicker
    /// exactly when the picture is already struggling.
    agc_gain: f32,
    agc_porch: f32,
...
/// A `min * 0.5` threshold referenced to zero does not hold here:
/// bright active video (e.g. a window in frame) biases the tip
/// position, progressively in the lower field rows — the "slanting
/// vertical line".
///
...
impl FrameReconstructor {
    ...
    pub fn reconstruct_frame_into(
        ...
        //
        // Fixed offsets from zero (`-0.3·rad_per_volt`, and `×0.8` of
        // it for the H-sync reject) are not usable here: they silently
        // assume the demod output arrives at exactly the nominal scale
        ...
        // `robust_sync_threshold_at`; 0.40 puts the tip→blanking
        // position where a fixed `h_sync_reject` would sit on a
        // nominally scaled signal.
        ...
        for row in 0..self.field_lines {
            ...
            // at f_sc. Rounding `current_sync_idx` and `next_sync_idx`
            // to integer samples instead makes the line read wander by
            // 0.5 samples per line — at 100 MSPS that is
            // 5 ns × 4.43 MHz × 2π ≈ 16° of chroma phase jitter per
            // line, which is what the per-row
        ...
        // fresh rather than guessing mid-field. This also structurally
        // avoids a search heuristic's failure mode (the leading
        // equalizing pulse look-alike, described below).
...
mod tests {
    ...
    /// blanking (PAL 25 lines, not a single hardcoded 20 for both), a
    /// purely vertical effect.
    #[test]
